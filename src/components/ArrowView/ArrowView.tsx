...
export default function ArrowView() {
  ...
  return (
    <div className="absolute z-50 flex justify-between w-full px-5 text-white top-1/2">
      <div
        className="flex items-center justify-center w-10 h-10 text-2xl text-black -translate-y-1/2 rounded-full cursor-pointer"
        onClick={() => handleArrowButtonPressed(ArrowType.Decrement)}
      >
        <AiOutlineArrowLeft />
      </div>
      <div
        className="flex items-center justify-center w-10 h-10 text-2xl text-black rotate-180 -translate-y-1/2 rounded-full cursor-pointer"
